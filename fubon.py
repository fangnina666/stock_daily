...
def main():
    ...
    parser.add_argument("--outfile", type=str, help="指定輸出的檔名（含路徑，程式會自動加上日期）")
    parser.add_argument("--stock", type=str, help="指定只輸出某一股票代號 (例如 2330)")
    parser.add_argument("--branch", type=str, help="指定分點代號，只抓該分點 (例如 9814)")
    ...
    # ★ 如果指定分點，只保留該分點
    if args.branch:
        broker_list = [b for b in broker_list if b["b"] == args.branch]
        print(f"📌 僅抓取分點 {args.branch}，共 {len(broker_list)} 個")
    if not broker_list:
    ...
    if args.stock:
        ...
        all_records = [r for r in all_records if r["股票代號"] == args.stock]
        after = len(all_records)
        print(f"📌 過濾股票 {args.stock}，保留 {after} 筆，原本 {before} 筆")
        if not all_records:
            print(f"⚠️ 股票 {args.stock} 沒有資料，跳過存檔")
            return
    ...
    if args.outfile:
        ...
        output_filename = base.parent / f"{stem}_{report_date}{suffix}"
    else:
        if args.stock:
            output_filename = report_dir / f"券商分點買賣超明細_{args.stock}_{report_date}.xlsx"
        else:
